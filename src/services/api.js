...
// If no backend URL is configured, run in demo mode
export const DEMO_MODE = !BASE_URL;

const api = axios.create({
  baseURL: BASE_URL || 'http://localhost:4000/api', // fallback never used in demo mode
  timeout: 10000,
});
...
api.interceptors.response.use(
  (res) => res.data,
  (err) => {
    // Normalise network / mixed-content errors into a readable message
    ...
    if (err.response?.data) {
      ...
      // don't surface raw protocol/URL strings like "Unsupported protocol HTTPS:"
      message = err.message;
    }
